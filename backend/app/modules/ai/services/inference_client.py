...
async def call_transcribe_from_url(audio_url: str) -> dict[str, Any] | None:
    """Descarga HTTPS breve y transcribe (p. ej. evidencia ya subida)."""
    if settings.AI_INFERENCE_STUB:
    ...
    async with httpx.AsyncClient(timeout=timeout, follow_redirects=True) as client:
        try:
            ...
            _log.warning("No se pudo descargar audio para IA: %s", e)
            return None
        ct = r.headers.get("content-type", "application/octet-stream")
        return await call_transcribe_audio(r.content, "evidencia.bin", ct.split(";")[0].strip())
