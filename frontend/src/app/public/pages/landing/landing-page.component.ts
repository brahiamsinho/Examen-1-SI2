...
export class LandingPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.publicApi.getStripeConfig().subscribe({
      next: (cfg) => {
        this.stripeEnabled = cfg.enabled;
      },
      error: () => {
    ...
    this.publicApi.listPricingPlans().subscribe({
      next: (rows) => {
        this.pricingPlans = rows
          .sort((a, b) => a.sort_order - b.sort_order)
          .map((p) => this.mapApiPlan(p));
        this.pricingLoading = false;
      },
      error: () => {
        this.pricingPlans = this.fallbackPricingPlans;
        this.pricingLoading = false;
      },
    });
  }
